...
windowSize = 60; % past 60 days as features
X_rf = [];
Y_rf = [];

for i = windowSize+1:length(trainOpening)
    % features: past 60 days original prices
    X_rf = [X_rf; trainOpening(i-windowSize:i-1)'];
...
%% Performance Evaluation
modelName = "Hybrid (ARMA+RF Residuals 10 days)";
fprintf("\nPerformance Evaluation for %s Model...\n", modelName);
...
%% Performance Evaluation
modelName = "Hybrid (ARMA+RF Residuals 5 days)";
fprintf("\nPerformance Evaluation for %s Model...\n", modelName);
