...
windowSize = 60; % Use the past 60 days to make predictions
x = [];
y = [];
...
for sim = 1:numSimulations
    ...
    y_sim = [];
    for i = 1:(length(diffTrainOpening) - windowSize)
        x_sim = [x_sim; diffTrainOpening(i:i+windowSize-1)']; % Features in the window size
        y_sim = [y_sim; diffTrainOpening(i+windowSize)]; % Target value: the price of the next day
    end
    ...
    % Convert forecasts back to the original scale
    lastOriginalValue_sim = trainOpening(end); % Last original value from the training set
    predictedRealPrices_sim = cumsum([lastOriginalValue_sim; predictedPrices_sim']); % Cumulative sum to restore original scale
    predictedRealPrices_sim = predictedRealPrices_sim(2:end); % Remove the initial value (start point)
    
    %% Performance Evaluation for 10 Days
    modelName_sim_10 = "Random Forest (diffData) 10 days - Simulation";
    testData_sim_10 = testOpening;
    predictedData_sim_10 = predictedRealPrices_sim;
    
    mse_sim_10 = mean((testData_sim_10 - predictedData_sim_10).^2);
    mse_10days(sim) = mse_sim_10;
    
    %% Performance Evaluation for 5 Days
    modelName_sim_5 = "Random Forest (diffData) 5 days - Simulation";
    testData_sim_5 = testOpening(1:5, :);
    predictedData_sim_5 = predictedRealPrices_sim(1:5, :);
...
fprintf("Average MSE over %d simulations for %s: %.4f\n", numSimulations, "Random Forest (diffData) 10 days", averageMSE_10days);
fprintf("Average MSE over %d simulations for %s: %.4f\n", numSimulations, "Random Forest (diffData) 5 days", averageMSE_5days);
